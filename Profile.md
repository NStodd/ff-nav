...
**A second action, "+ STOP" (Direction F, phase 3), sits next to "GO"** on each saved place — shown only once a trip is already active (`navigation.hasRoute`), since a waypoint only means anything relative to a destination that's already set. Where "GO" replaces the current trip entirely, "+ STOP" calls `navigation.addWaypoint()` and adds the saved place as an intermediate stop on the trip already in progress — the other way (besides arming "+ ADD STOP" on the map itself) to build a multi-stop route. See `Navigation.md`'s "Multi-stop routes" section for the mechanics.
