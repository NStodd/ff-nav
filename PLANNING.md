...
### 31. Genre world-skinning, phase 1: `/dev/map` palette picker + FF pilot

**Files created:** `src/data/mapLayerGroups.js`, `src/data/worldSkins.js`
**Files changed:** `src/views/MapPlayground.vue`

First concrete step of §9, exactly as its own implementation plan laid out: extend `/dev/map` with a genre-palette picker before touching the real map screen at all, and pilot on FF only.

**`mapLayerGroups.js`** is the real, fetched-and-inspected id list §9 called for, not a guess — Dark Matter's actual style JSON has 93 layers, and this groups the ones worth reskinning into `background`/`water`/`waterway`/`labelWater`/`land`/`buildingTop`/`roadMajor`(+`roadMajorCase`)/`roadMinor`(+`roadMinorCase`)/`labelPlace`/`labelRoad`. Deliberately not exhaustive: rail, administrative boundaries, and house numbers carry little visual weight and stay default for every genre; the base `building` layer (as opposed to `building-top`) is skipped on purpose too, since its fill-color is a zoom-dependent stops array that fades in from `transparent` — a flat override would remove that behavior, not just recolor it.

**`worldSkins.js`** holds `WORLD_SKINS`, one palette object per genre id, plus `applyWorldSkin(map, groups, skin)` — a small, defensive function (each `setPaintProperty` call wrapped individually so one unsupported property doesn't abort the rest) that any screen can call once its style has loaded. Only `ff` is populated for this pass; the other three keys are intentionally absent, so `worldSkinFor(genreId)` returning `null` for them is itself the "not designed yet" signal, not a bug to fix later.

**The FF palette itself**, a "moonlit fantasy map" direction — warm torchlit hues in place of the shipped theme's neutral cool grays, while staying exactly as dark. That last part is deliberate, not a compromise: Milestone 6's own lesson (high contrast and minimal visual noise are safer to read at a glance while driving) applies just as much to a reskinned map as it did to the plain one, so this isn't a bright "old parchment" look. Background shifts from neutral near-black to a warm umber-black, water from cool blue-gray to a deep enchanted-lake teal, roads from cool grays to gold-brown (with a darker brown casing), buildings to a warm brown, and labels to a parchment-gold that ties back to the app's existing `--ff-gold` family.

**`MapPlayground.vue`** gained a "World skin" section above the existing route-line-treatment picker: five buttons, one per genre plus "Default (shipped)". FF and Default are clickable; the other three render disabled with a visible "(not yet designed)" suffix, so the picker itself documents where §9 currently stands rather than just omitting genres that aren't ready. Selecting a skin calls `map.setStyle()` (a full style reload, not a partial mutation) and re-applies the chosen skin plus rebuilds the route line on the resulting `style.load` — the same "rebuild rather than partially toggle" approach the route-line gradient treatment already established for this file, needed here because `setStyle()` wipes any source/layer the tool added. Markers didn't need any special handling: they're positioned by the `Map` instance directly, not by the style, so they survive a style swap untouched.

**Verified against the real style, not just code review**: fetched and inspected the live Dark Matter style JSON directly (93 layers, exact paint values for every targeted layer) before writing the palette rather than guessing at plausible-looking hex values. Playwright-confirmed the picker's button states (FF enabled, other three disabled with the correct label) and zero console/page errors through a full default → FF → default cycle. Screenshotted the map region itself (not the whole tool) at both a desktop width and — catching that the plan's own verification bar calls for phone-realistic sizing, not just any screenshot — a real 390px-wide crop matching a phone's map viewport: street names, the route line, and the destination marker all stayed fully legible and unaffected by the reskin at that width. One small, honestly-caught gap, not swept under the rug: named park/stadium labels (`poi_park`/`poi_stadium`, a different source-layer than the generic `place_*` names `labelPlace` already covers) still render in the default cool color rather than the FF palette — a minor consistency gap for a later refinement pass, not a functional or safety problem, since it's two label layers out of the whole map.
...
- **Settled: "genre feel" extends beyond palette** — road *line style* (dashed/organic edges reading as "old map" for fantasy vs. crisp grid lines for sci-fi), *land type* variation (not just one flat land color), genre-specific *POI icon* sets, and *HUD chrome* treatment are all real, wanted extensions, not scope creep if they show up later. Explicitly not required for the first palette-comparison screenshots, though — each is its own design surface, deserving its own pass once this phase's base mechanism is proven. `worldSkins.js` says as much in its own header comment so this doesn't get forgotten as "done" once palette alone ships.
- **POI icons stay universal for now** — a café is a café regardless of genre; the existing category-based set (`poiIcons.js`) isn't touched by this phase, and genre-specific icon variants (if wanted per the above) are follow-up work, not part of the palette mechanism itself.
