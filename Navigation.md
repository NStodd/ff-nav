...
- **Route line** — a GeoJSON source (`route`) + line layer (`route-line`) added once in the map's `load` event, painted in `store.chosenClass.color`. `syncRoute()` just calls `.setData()` on the existing source whenever `navigation.route` changes; the source/layer themselves are never recreated. A directional treatment (gradient or leading pulse showing which end is "ahead") was scoped in Milestone 6 but deliberately deferred — see that section's own note.
