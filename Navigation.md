...
| `waypoints` | `[{ id, lat, lng }, ...]` | Intermediate stops between `position` and `destination`, in visiting order (Direction F, phase 3). Persisted the same way (`crystalpath-waypoints`) and for the same reason. `destination` stays the single final stop everywhere else already assumes one — this is additive, not a replacement of that concept with an array. |
...
- `setDestination(latLng, prefs = {})` — stores the destination (persisted, see the state table above), **clears `waypoints`** (a fresh destination starts a fresh trip — stops planned for the previous one don't carry over), and, if a position is already known, immediately calls `fetchRoute()`. If no position is known yet, the destination is stored and the route fetch happens later — see `MapScreen.vue`'s position watcher below, which now catches up on exactly this case.
- `addWaypoint(latLng)` / `removeWaypoint(id)` (Direction F, phase 3) — add/remove an intermediate stop and immediately re-fetch. `addWaypoint` no-ops without a destination already set (a stop only means anything relative to a trip that's already going somewhere). See "Multi-stop routes" below.
- `fetchRoute(origin, dest, prefs = {}, _isRetry = false)` — calls the public OSRM demo server with `alternatives=true&steps=true`, now over **every** stop in order (`origin;waypoint1;waypoint2;...;dest`, built by the module-private `buildCoordsString()`), and picks among the returned routes via `pickRoute()` (see below) instead of always taking the first one, then sets `route`/`eta`/`routeDistanceMeters` from the pick. Guarded by `fetchingRoute` against overlapping calls. Wrapped in try/catch (previously unguarded — a network failure or malformed response threw uncaught): on a non-OK response or an empty `routes` array it throws, is caught, and retries once after `ROUTE_RETRY_DELAY_MS` (1.5s); a second failure sets `routeError` instead of throwing again. `_isRetry` is an internal recursion flag, not part of the call signature any caller should pass.
...
### Multi-stop routes (Direction F, phase 3)

`waypoints` (`[{ id, lat, lng }, ...]`) holds intermediate stops between `position` and `destination`, in visiting order. `destination` stays the single final stop — everywhere else that already assumes exactly one (the destination marker, the saved-destination star, arrival XP) needed no change, since this is additive rather than a `destination`-becomes-an-array rewrite.

**OSRM already does the hard part.** `/route/v1/driving/{p1};{p2};{p3};...` is a normal, documented OSRM request — the response comes back with one `legs[]` entry per hop, and `analyzeRoute()`/`flattenSteps()` were already written to walk `route.legs` generically (confirmed against a live 3-point request before writing any new code: `legs.length === 2` for 3 coordinates, exactly as expected). So turn-by-turn needed **zero changes** to work correctly across multiple legs — `currentStepIndex`'s threshold-based advance doesn't know or care about leg boundaries, it just walks a flat array of maneuver points, and a multi-leg route's steps flatten into that same array the same way a single-leg one always did.

**What actually changed:** `buildCoordsString(origin, waypointList, dest)` (module-private) builds the full semicolon-separated coordinate string; `fetchRoute()` and `attemptReroute()` both use it instead of a hardcoded two-point URL. `addWaypoint(latLng)` appends and re-fetches (no-ops without a destination — a stop only means something relative to a trip already headed somewhere); `removeWaypoint(id)` removes and re-fetches. Both need *some* `prefs` to pass `pickRoute()` for that re-fetch, but this store still never imports `player.js` for `store.preferences` — instead, `fetchRoute()`/`attemptReroute()` record whatever `prefs` they were most recently called with into a closure variable (`lastPrefs`, not a ref — nothing outside this store needs to read it), and the two internally-triggered refetches below reuse that. Every *caller-initiated* action (a real map tap, a real ability press) still gets its `prefs` passed in explicitly by the view layer, same as always; `lastPrefs` only covers the refetches this store decides to make on its own.

**Reaching an intermediate stop is not finishing the trip.** A second arrival watcher (`watch([position, waypoints], ...)`) checks proximity to `waypoints[0]` the same way the existing final-arrival watcher checks `destination`, but on reaching it just drops it from the list and re-fetches toward whatever's left — no XP, no `tripJustCompleted`, the same "no popup, just keep going" spirit as the Speedrunner archetype's silent reroute. Only reaching the *final* `destination` completes a trip.

Persisted the same way `destination` is (`crystalpath-waypoints`), for the same reason: a reload mid-multi-stop-trip should restore the plan, not silently drop it to a single leg. Cleared alongside `destination` everywhere that already clears it: final arrival, `goDark()`, `reset()`, and a fresh `setDestination()` call.
...
- **Setting a destination** — a `map.on('click', ...)` handler that calls `navigation.setDestination(...)` by default, or `navigation.addWaypoint(...)` instead when a local `addingStop` ref is armed via the "+ ADD STOP" toggle (Direction F, phase 3) — the click disarms itself afterward. There's no destination search/POI picker yet; clicking the map *is* the interaction for now.
- **Waypoint markers** — numbered circles (`waypointMarkerEl()`), reconciled by `syncWaypoints()` the same full-add/remove-sync pattern `syncPOIs()` already uses for `pois`. Order is the one thing a stop marker needs to convey — the route line itself already shows the path between them — so an existing marker's label is updated in place rather than recreated when an earlier stop's removal shifts every later number down.
