...
- **Real quests and pickups, not busywork.** Every quest objective tracks something the app already measures — trips taken, distance covered, abilities used, real places found — auto-completed and rewarded the instant you cross the line, no separate claim step. Reaching a revealed point of interest collects it as a pickup, closing the loop on scouting one in the first place. Five quests per genre, all four genres, from day one.
- **Titles that are a status, not an event.** Five genre-agnostic badges (Pathfinder, Wanderer, Adept, Collector, Master Cartographer) recomputed fresh every time your profile renders, from the same lifetime stats quests already track — nothing about a title is ever separately stored.
- **The map itself starts to reskin by genre, not just the character layer.** Final Fantasy's map now repaints the same real tiles in warm, torchlit hues instead of the shipped neutral grays — same street data, same POI positions, same routing, only the color each layer draws in changes. The other three genres are next; see [`PLANNING.md`](PLANNING.md) §9.
...
- **Saved destinations, turn-by-turn, and multi-stop routes.** Save your current destination with a tap from the map and pick it back up later from your profile screen; once a route is active, a persistent HUD strip shows the next real maneuver (from OSRM's own step data) with a live countdown distance, not just an ETA; and a trip can have more than one stop — add one from the map or from a saved place, each dropping off the plan automatically once you reach it.
